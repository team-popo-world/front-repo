...
import character2_img from "../../assets/image/raising/raising_character2.webp";
import character3_img from "../../assets/image/raising/raising_character3.webp";
import character4_img from "../../assets/image/raising/raising_character4.webp";
import character5_img from "../../assets/image/raising/raising_character5.webp";
...
import fish_img from "../../assets/image/raising/raising_fish.webp";
...
export default function RaisingPage() {
  ...
  const [feedList, setFeedList] = useState([
    ...
    { name: "bread", img: bread_img, count: 8 },
    { name: "fish", img: fish_img, count: 7 },
    { name: "apple", img: apple_img, count: 2 },
  ...
  function getCharacterImg(level: number) {
    if (level >= 5) return character5_img;
    if (level == 4) return character4_img;
    if (level == 3) return character3_img;
    if (level == 2) return character2_img;
    return character1_img;
  }
  ...
  function FeedModal({
    ...
    onCancel: () => void;
    onConfirm: (selectedFeeds: string[]) => void;
  }) {
        ...
        {levelUp && (
          <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/30">
            <div className="bg-[#FFF6D1] rounded-2xl px-8 py-6 shadow text-center flex flex-col items-center">
              ...
              <img
                src={getCharacterImg(level + 1)}
                alt={`character${level + 1}`}
                className="w-40 h-40 mx-auto mb-2"
              />
              <div className="text-xl font-bold text-[#6F4223] mb-2">
                포포가 성장했어요!
              </div>
            </div>
          </div>
        ...
        {showExpMsg && !levelUp && (
          <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/30">
            <div className="bg-[#FFF6D1] rounded-2xl px-8 py-3 shadow text-center">
              <div className="text-2xl font-bold text-[#6F4223] mt-1">
                +{addedExp} 경험치 획득!
              </div>
            </div>
          </div>
