...
export const TurnFinishPigCard = ({ image, name, priceChange, countChange }: PigCardProps) => {
  return (
    <div className="flex flex-col px-1 items-center">
      <img src={image} alt={name} className="mb-3 min-w-0 h-16 object-contain" />
      <h4 className="text-main-brown-575 text-[0.75rem] font-bold mb-1 self-center">{name}</h4>
      <div className="flex flex-col gap-y-0.5 mt-1 ">
        <div className="flex items-center gap-x-0.5">
          <img src={coin} alt="코인" className="w-4 h-4 object-contain" />
          <span className="text-main-brown-575 text-[0.75rem] font-bold">가격 변동: </span>
          <span
